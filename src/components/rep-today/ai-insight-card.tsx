...
import { AI_INSIGHT, type PremiseStat } from "@/lib/rep-today-data";
...
export function AiInsightCard() {
  return (
    <SectionCard
      icon={Sparkles}
      title="AI Insight"
      description={AI_INSIGHT.summary}
...
export function PremiseRow() {
  return (
    <View className="flex-row gap-3">
      <PremiseCell stat={AI_INSIGHT.onPremise} />
      <PremiseCell stat={AI_INSIGHT.offPremise} />
    </View>
  );
}

/** Standalone card holding the example-data note. */
export function InsightNoteCard() {
  return (
    <View className="rounded-2xl bg-white/5 p-4">
      <Text className="text-sm text-muted-foreground">{AI_INSIGHT.note}</Text>
    </View>
  );
}
